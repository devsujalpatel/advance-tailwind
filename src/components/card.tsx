...
export const Card = () => {
  ...
  return (
    <div className="relative z-10 h-80 w-60 overflow-hidden rounded-xl border dark:border-neutral-800 border-neutral-100 bg-neutral-50 shadow-2xl dark:bg-neutral-900">
      <div className="relative mask-r-from-50% mask-l-from-50%">
        <Pattern />
        <div className="animate-marquee flex h-40 items-center justify-center gap-4">
          {icons.map((item, index) => (
            <div
              key={index}
              className={`flex size-10 items-center justify-center rounded-full bg-neutral-300`}
            >
              {item.icon}
            </div>
          ))}
      ...
      <div className="p-4">
        <h2 className="text-sm font-bold text-neutral-600 dark:text-neutral-200">
          These LLMs are getting <br /> out of hand
        </h2>
        <p className="mt-2 text-[10px] text-neutral-400">
          I swear god there's always another AI dropping and twitter goes crazy.
          It's all a myth guys. Like seriosly, one day it's Claude 2, then GPT-4
          Turbo, When will it end? They're all just fancy autocomplete.
        </p>
      </div>
    </div>
  );
